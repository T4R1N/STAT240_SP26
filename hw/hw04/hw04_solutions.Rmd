...
What do each of these four quantities (`w`, `x`, `y`, `z`) represent in real terms? Do not use technical vocabulary.


- [`w` is the number of zip codes in Wisconsin.]{.soln}

- [`x` is the number of zip codes in which the percentage of women with a bachelors degree is higher than the percentage of men with a bachelors degree.]{.soln}

- [`y` is the percentage of zip codes in which the percentage of women with a bachelors degree is higher than the percentage of men with a bachelors degree.]{.soln}

- [`z` is the number of zip codes in which both the percentage of women with a bachelors degree and the percentage of men with a bachelors degree are unknown.]{.soln}
...
[Points below the red line represent a zip code in which the percentage of women with a bachelors degree is higher than the percentage of men with a bachelors degree.]{.soln}
...
- Remove all rows where there are 0 estimated obese individuals. (Bonus question: Why didn't we just do this at the beginning?)
...
[Answer to bonus question: We needed to know the `pop` from every single row to calculate total population of a zip-sex combo, even if there were no obese people in that row. If we would have cleared out the 0 obese rows early, we would have lost some population.]{.soln}
...
```{r, class.source="rsoln", class.output="rsoln"}
joined <- inner_join(education_long, obesity_summary,
                     join_by(zip, sex))
```
...
[As the percentage of adults with a bachelor's degree goes up, the percentage of adults with obesity tends to decrease.]{.soln}
