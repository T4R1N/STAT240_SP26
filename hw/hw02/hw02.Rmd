...
It will make a straight line from a linear model instead of a curved one.
...
x = year 1,
y = duration;
These are global since they're put as parameters of the ggplot function call.

color = red,
color = blue;
These are local since they're put as parameters of individual geom functions

They are all constants, and attempting to change them would cause an error.
...
```{r}
ggplot(mapping = aes(x = year1, y = duration, color = lake)) +
...
```{r}
ggplot(mapping = aes(x = year1, y = duration, alpha = 0.7, color = lake)) +
  geom_line(data = combined) 
  # geom_smooth(method = 'lm', data = combined)
...
There is a clear area where lake Mendota's average is above Monona's in problem 6, whereas it's possible but far more difficult to do so in problem 7.
