...
```{r}
mass_by_year %>%
  ggplot(aes(year, minimum_mass)) +
  scale_y_log10() +
  geom_point() +
  labs(title = "Exoplanet Minimum Mass by Year", x = "Year", y = "Minimum Mass")
```
...
The volume of a planet is $\frac{4}{3} \times \pi \times \text{radius}^3$. Note that `pi` is a constant built into R that you may use.

```{r}
planet_densities <- planets %>%
  mutate(volume = (4/3) * pi * radius ^ 3, density = mass / volume) %>%
  arrange(-density)

print(planet_densities, n = 5)
```
...
```{r}
planets %>%
  filter(!is.na(radius) & radius < 40) %>%
  ggplot(aes(method, radius, color = method)) +
  geom_boxplot()
```

According to the box plots, Transit captured the smallest radius planet within its first quartile.
...
print(methods_within_year, n = 20)
```
...
```
```{r}
# methods_within_year <- methods_within_year %>%
year_total <- methods_within_year %>%
  group_by(year) %>%
...
print(methods_within_year_proportions, n = 10)
```
...
methods_within_year_proportions %>%
  ggplot(aes(year, methodProportion)) +
  geom_col(aes(fill = method))
```

In the early 2000's, Radial Velocity was the dominant method of planet discovery. However, by the 2010's, a variety of new methods were being used, and Transit took the lead as the new dominant method.
