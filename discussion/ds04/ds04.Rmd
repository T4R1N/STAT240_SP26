...
```{r}
donations <- donations %>% 
  mutate(caloric_density = weight*4.54)
...
```{r}
donations_summary %>%
  group_by(species, species_diet) %>%
  summarize(species_donations = mean(total_donations)) %>%
  group_by(species_diet) %>%
  summarize(diet_donations = mean(species_donations))
```
...
long_term_donations %>%
  group_by(species_diet) %>%
  summarize(average_density = mean(caloric_density))
```
...
donations_by_type %>%
  group_by(food_type) %>%
  summarize(herbi_omni_ratio = herbivore / omnivore) %>%
  group_by(food_type, herbi_omni_ratio) %>%
  summarize(herbivores_more = herbi_omni_ratio > 1)
```
